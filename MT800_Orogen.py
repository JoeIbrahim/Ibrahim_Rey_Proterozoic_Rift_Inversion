...
#This script is used to run the MT800 experiment. Start with this script to create the initial wide rift. Use the other scripts to restart this experiment under tectonic quiescence or contraction.
...
# %%
### Extensional Velocity Boundary Conditions
velocity = 1.073 * u.centimeter / u.year

# Use the following lines for extensional boundary conditions:
...
# Use the following lines for tectonically quiescent boundary conditions:
...
### Run the model. 
# restartStep = -1 to restart from the latest checkpoint. You can also specify a particular checkpoint number. E.g. to begin a phase of tectonic quiescence after 8 m.y. of extension, use restartStep=80, comment out the extensional boundary conditions above, and uncomment the quiescence boundary conditions. 
# The restartDir should be the name of the output directory used in the previous run. E.g., to restart from the end of the Rift_800C model, use restartDir="MT800C_Rift_O", to restart this model use the current output directory name "MT800_Qsc_O".
Model.run_for(60000001 * u.years, checkpoint_interval=100000. * u.year, restartStep=-1, restartDir="MT800C_Rift_O")
