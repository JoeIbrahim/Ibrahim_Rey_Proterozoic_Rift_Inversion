...
#This script is used to run the MT600 experiment. Start with this script to create the initial rift. Use the other scripts to restart this experiment under tectonic quiescence or contraction.
...
### Model resolution and scaling coefficients
...
#Output directory for model files. Underworld will create this directory if it does not already exist.
Model.outputDir="MT600_Orgn_O"
...
uppermantle = Model.add_material(name="Upper Mantle", shape=GEO.shapes.Layer(top=-36. * u.kilometer, bottom=-125. * u.kilometer))
...
asthenosphere = Model.add_material(name="Asthenosphere", shape=GEO.shapes.Layer(top=-125. * u.kilometer, bottom=Model.bottom))
...
# %%
Model.set_heatFlowBCs(bottom=(0.024 * u.watt / u.metre**2, asthenosphere))
...
# %%
### Compressional Velocity Boundary Conditions
...
### Seeding initial plastic strain
...
#### Resetting temperature boundary conditions after model initialization 
Model.init_model()
...
### Post Hook for plastic strain seeding
def post_hook():  
...
### Run the model. 
# Specify a particular checkpoint number to restart from. E.g. to begin a phase of contraction after 8 m.y. of extension, use restartStep=80, comment out the extensional boundary conditions above, and uncomment the quiescence boundary conditions. 
# The restartDir should be the name of the output directory used in the previous run. It is currently set to restart from the end of the quiescence phase. E.g., to restart from the Rift_600C model, use restartDir="MT600C_Rift_O", to restart this model use the current output directory name "MT600_Orgn_O".
Model.run_for(8000001 * u.years, checkpoint_interval=100000. * u.year, restartStep=-1, restartDir="MT600C_Qsc_O")
